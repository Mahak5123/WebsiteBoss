...
const BusinessInfoPage = () => {
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      const selectedIndustry = sessionStorage.getItem('selectedIndustry') || 'Not specified'; // ✅ safely retrieved
      const fullData = {
        ...formData,
        colorTheme: selectedTheme,
        
      selectedIndustry, // ✅ pull actual value from sessionStorage
      };
      
   // ✅ Save both businessData and selectedIndustry separately
    sessionStorage.setItem('businessData', JSON.stringify(fullData));
      alert('Business information saved successfully!');
      console.log('Navigation to /product-setup would occur here');
  ...
  const styles = {
    ...
    colorBox: (theme) => ({
      padding: '15px',
      borderRadius: '10px',
      border: selectedTheme === theme ? '3px solid #8b5fbf' : 'none',
      cursor: 'pointer',
      textAlign: 'center',
      background: selectedTheme === theme ? 'linear-gradient(45deg, #8b5fbf, #6a4c93)' : 'linear-gradient(45deg, #342447, #2a1f3d)',
      color: selectedTheme === theme ? '#e0d4f7' : '#c794f7',
      flex: 1,
    }),
  };
      ...
      <div style={styles.formContainer}>
        <div>
          {Object.entries(formData).map(([key, value]) => (
            <div key={key}>
              <label style={styles.label} htmlFor={key}>{key.replace(/([A-Z])/g, ' $1').replace(/^\w/, c => c.toUpperCase())}</label>
              {key === 'businessDescription' || key === 'servicesOffered' || key === 'targetAudience' || key === 'address' ? (
  <textarea
    style={{
      ...styles.input,
      ...styles.textarea,
      color: '#c794f7'  // <- Make textarea text match label color
    }}
    id={key}
    name={key}
    value={value}
